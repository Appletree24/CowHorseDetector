...
fn run() -> Result<()> {
    ...
    if commits.is_empty() {
        println!(
            "No commits found in {} for the selected filters.",
            repo_path.display()
        );
        return Ok(());
    }

    let metrics = compute_metrics(&repo_path, &commits);
...
#[command(
    name = "cow-horse",
    about = "Measure how \"cow-horse\" (aka overworked) a repository feels based on its git history."
)]
struct Cli {
...
fn compute_metrics(repo_path: &Path, commits: &[Commit]) -> RepoMetrics {
    ...
    RepoMetrics {
        ...
        severity_score,
        severity_label,
        top_after_hours_authors: author_summaries,
    }
}
...
fn severity_label(score: f64) -> &'static str {
    match score as u32 {
        0..=20 => "Chill",
        21..=40 => "Healthy-ish",
        41..=60 => "Sustained overtime",
        61..=80 => "Horse mode",
        _ => "Full cow-horse alert",
    }
}
...
fn print_human_report(metrics: &RepoMetrics, cli: &Cli) {
    println!("Repository: {}", metrics.repo_path.display());
    ...
    if cli.author.is_some() {
        println!("Author filter: {}", cli.author.as_deref().unwrap());
    }

    println!(
        "Commits analyzed: {} ({} authors, {} active days)",
        metrics.total_commits, metrics.unique_authors, metrics.commit_days
    );
    println!(
        "Cow-horse score: {:>5.1}/100 -> {}",
        metrics.severity_score, metrics.severity_label
    );
    println!(
        "After-hours commits: {} ({:.1}%)",
    ...
    if !metrics.top_after_hours_authors.is_empty() {
        ...
        for author in &metrics.top_after_hours_authors {
            println!(
                "  - {} -> {} commits | {:.1}% after-hours | {} weekend | {} deep-night",
                author.name,
